...
export default function ProductsPage() {
  ...
  const getSeasonalDisplayPrice = (product: Product, monthIndex: number) =>
    getProductDailyPrice(product, new Date(2026, monthIndex, 1));
        ...
        {products.map((product) => (
          <div key={product.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden flex flex-col">
            <button
              ...
              className="h-48 bg-gray-100 relative text-left cursor-pointer"
            ...
            <div className="p-4 flex-1 flex flex-col">
              ...
              <div className="flex items-center justify-between mt-auto pt-4 border-t border-gray-100">
                  ...
                  {getConfiguredMonths(product).length > 0 && (
                    <div className="mt-3">
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        {getConfiguredMonths(product).map(({ key, label, monthIndex }) => (
                          <div
                            key={key}
                            className="rounded-full border border-gray-200 bg-gray-50 px-3 py-1.5 text-[11px] text-gray-700"
                          >
                            {label}: €{formatPrice(getSeasonalBasePrice(product, monthIndex))}
                          </div>
                        ))}
                      </div>
                    </div>
                ...
                <div className="flex gap-2">
                  <button 
                    onClick={() => handleEdit(product)}
                    className="btn-icon p-3 text-blue-600 hover:bg-blue-50"
                    title="Editar"
                  >
                    <Edit size={18} />
                  </button>
                  <button 
                    ...
                    className="btn-icon p-3 text-rose-600 hover:bg-rose-50"
                    title="Eliminar"
                  >
                    <Trash2 size={18} />
                  </button>
